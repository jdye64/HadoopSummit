#!/bin/bash
SLEEP_SEC="30"
HDP_IMAGE_NAME="jdye64/hadoopsummit:summit_osx"
...
CONTAINER_ID=$(docker ps | grep jdye64/hadoopsummit:summit_osx | awk '{ print $1 }')
...
CONTAINER_ID=$(docker run -t -d -p 8080:8080 --privileged -v /dev/video0:/dev/video0 -h docker.dev $HDP_IMAGE_NAME)
...
NIFI_URL="http://$IP_ADDR:8080/nifi"
